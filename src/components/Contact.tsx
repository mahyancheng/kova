      ...
      <div className="relative max-w-[1240px] mx-auto px-6 lg:px-10">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-y-10 lg:gap-x-16 lg:items-start">
          ...
          <div className="lg:col-span-6 lg:row-start-1 lg:col-start-1">
            ...
            <h1 className="mt-4 headline fluid-h2 font-bold not-italic text-[var(--color-cream)]">
              {t.contact.titleA}{" "}
            ...
            <div className="mt-5 inline-flex items-center gap-2.5 px-3.5 py-2 rounded-full border border-[var(--color-cream)]/20 bg-white/[0.04]">
              ...
              <span className="text-[0.78rem] tracking-tight text-[var(--color-cream)]/85">
          ...
          <div className="row-start-2 lg:row-start-1 lg:row-span-2 lg:col-start-7 lg:col-span-6 lg:pl-10 lg:border-l lg:border-[var(--color-cream)]/15">
            {sent ? (
              ...
              <form onSubmit={handleSubmit}>
                ...
                {/* Message */}
                <div className="mt-5">
                  <label className="block text-[0.68rem] tracking-widest uppercase text-[var(--color-sand)] mb-2">
                  ...
                  <textarea
                    ...
                    className="w-full bg-transparent border-b border-[var(--color-cream)]/25 focus:border-[var(--color-clay-light)] outline-none py-2 text-[0.88rem] text-[var(--color-cream)] placeholder:text-[var(--color-cream)]/35 resize-none transition-colors"
                  />
                </div>

                {/* Interest */}
                <fieldset ref={interestRef} className="mt-5">
                  <legend className="block text-[0.68rem] tracking-widest uppercase text-[var(--color-sand)] mb-2">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {t.contact.interests.map((option) => (
                      <label
                        key={option}
                        className="cursor-pointer px-3 py-1.5 rounded-full border border-[var(--color-cream)]/25 text-[0.88rem] text-[var(--color-cream)]/85 hover:bg-[var(--color-cream)]/10 has-[:checked]:bg-[var(--color-clay)] has-[:checked]:border-[var(--color-clay)] has-[:checked]:text-[var(--color-cream)] transition-colors"
                      >
                ...
                <div className="mt-6">
                  <button
                    type="submit"
                    disabled={submitting}
                    className="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-6 py-3.5 sm:py-3 rounded-full bg-[var(--color-clay)] text-[var(--color-cream)] font-medium text-[0.88rem] sm:text-[0.88rem] hover:bg-[var(--color-clay-deep)] active:bg-[var(--color-clay-deep)] transition-colors disabled:opacity-60 disabled:cursor-wait"
                  >
          ...
          <div className="row-start-3 lg:row-start-2 lg:col-start-1 lg:col-span-6">
            ...
            <dl className="grid grid-cols-2 gap-y-5 gap-x-8 max-w-md text-[0.88rem]">
              <div>
                <dt className="text-[var(--color-sand)] text-[0.68rem] tracking-widest uppercase">
                  {t.contact.studio}
                </dt>
  ...
  return (
    <div>
      <div className="flex items-baseline justify-between gap-3">
        <label
          htmlFor={name}
          className="block text-[0.68rem] tracking-widest uppercase text-[var(--color-sand)]"
        >
          {label}
          {primary && (
            <span
        ...
        {optionalLabel && (
          <span className="text-[0.68rem] tracking-tight text-[var(--color-cream)]/45 lowercase">
            {optionalLabel}
          </span>
        )}
      </div>
      ...
      />
      {hint && (
        <p className="mt-1.5 text-[0.68rem] text-[var(--color-cream)]/55 leading-snug">{hint}</p>
      )}
    </div>
  );
}