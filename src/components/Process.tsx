          ...
          <ol className="lg:col-span-8 grid sm:grid-cols-2 gap-px bg-[var(--color-line)] border border-[var(--color-line)] rounded-sm overflow-hidden">
            {t.process.steps.map((s) => (
              <li key={s.n} className="bg-[var(--color-cream)] p-[clamp(1.25rem,0.8rem+1.2vw,2rem)]">
                ...
                <h3 className="mt-1.5 lg:mt-2 font-serif text-[clamp(1.05rem,0.95rem+0.5vw,1.3rem)] leading-snug tracking-tight text-[var(--color-ink)]">
                  {s.title}
                </h3>
                <p className="mt-2 lg:mt-3 text-[0.88rem] leading-relaxed text-[var(--color-muted)]">
                  {s.body}
                </p>
              </li>
            ))}
