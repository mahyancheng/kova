...
export function ProductSpotlight({
  ...
  return (
    ...
    >
      <div className="max-w-[1240px] mx-auto px-6 lg:px-10 pt-16 lg:pt-24">
        <Reveal>
      ...
      <div className="max-w-[1240px] mx-auto px-6 lg:px-10 pt-6 lg:pt-10 pb-16 lg:pb-24">
        <div className={cn("grid lg:grid-cols-12 gap-10 lg:gap-12 items-end", reverse && "lg:[&>div:first-child]:order-2")}>
          <div className="lg:col-span-7">
            <Reveal>
              <h2 className={cn("headline text-[2.7rem] sm:text-[3.6rem] lg:text-[5rem]", isInk ? "text-[var(--color-cream)]" : "text-[var(--color-ink)]")}>
                {taglineA}
          ...
          <div className="lg:col-span-5">
            <Reveal delay={100}>
              <div className={cn("space-y-4 text-[1.02rem] leading-[1.65]", isInk ? "text-[var(--color-cream)]/80" : "text-[var(--color-ink-soft)]")}>
                {body.map((p, i) => (
                  <p key={i}>{p}</p>
                ))}
              </div>
      ...
      <div id={`${id}-spec`} className="max-w-[1240px] mx-auto px-6 lg:px-10 py-20 lg:py-28">
        <div className="grid lg:grid-cols-12 gap-10 lg:gap-16">
          ...
          <div className="lg:col-span-7">
            <Reveal>
              ...
              <h3 className={cn("mt-4 headline text-[1.9rem] sm:text-[2.4rem] lg:text-[2.8rem]", isInk ? "text-[var(--color-cream)]" : "text-[var(--color-ink)]")}>
                {c.whyTitle}
              </h3>
            </Reveal>
            ...
            <ol className="mt-12 space-y-12">
              {features.map((f, i) => (
                <Reveal key={i} delay={i * 50}>
                  <li className={cn("relative pl-14 sm:pl-20", isInk ? "border-l border-white/10" : "border-l border-[var(--color-line)]")}>
                    <span
                      className={cn(
                        "absolute -left-px top-1 font-serif text-[0.9rem]",
                    ...
                    <h4 className={cn("font-serif text-[1.4rem] sm:text-[1.6rem] leading-tight tracking-tight", isInk ? "text-[var(--color-cream)]" : "text-[var(--color-ink)]")}>
                      {f.title}
                    </h4>
                    <p className={cn("mt-3 text-[1rem] leading-[1.7] max-w-prose", isInk ? "text-[var(--color-cream)]/70" : "text-[var(--color-muted)]")}>
                      {f.detail}
                    </p>
                  </li>
                </Reveal>
            ...
            <Reveal>
              <div className={cn("mt-16 pt-10 border-t", isInk ? "border-white/10" : "border-[var(--color-line)]")}>
                ...
                <ul className="mt-4 flex flex-wrap gap-2">
                  {perfectFor.map((p) => (
                    <li
                      key={p}
                      className={cn(
                        "px-3.5 py-1.5 rounded-full text-[0.86rem] transition-colors border",
                        isInk
